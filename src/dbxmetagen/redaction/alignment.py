...
def align_entities_row(
    ...
    Returns:
        List of aligned entities with confidence scores and entity_type

    Example:
        >>> ai_ents = [{"entity": "John", "start": 0, "end": 3, "entity_type": "PERSON"}]
    ...
    for ai_ent in ai_entities:
        ...
        for i, pres_ent in enumerate(presidio_entities):
            ...
            # Exact match
            if (
                str(ai_ent.get("entity")) == str(pres_ent.get("entity"))
                and int(ai_ent.get("start")) == int(pres_ent.get("start"))
                and int(ai_ent.get("end")) == int(pres_ent.get("end"))
            ):

                best_match = pres_ent
                presidio_score = (
                    float(pres_ent.get("score"))
                    if pres_ent.get("score") is not None
                    else None
                )
                confidence = "high"
                used_presidio.add(i)
            ...
            # Check for overlapping match with fuzzy text similarity
                ...
                int(ai_ent.get("start")),
                int(ai_ent.get("end")),
                int(pres_ent.get("start")),
                int(pres_ent.get("end")),
            ) and is_fuzzy_match(
                ...
                if len(str(pres_ent.get("entity"))) > len(str(ai_ent.get("entity"))):
                    best_match = pres_ent
                    confidence = (
                        "medium" if presidio_score and presidio_score >= 0.7 else "low"
                    )
                    used_presidio.add(i)
                else:
                    confidence = "medium"
        ...
        # Determine entity_type: prefer AI entity_type, fall back to Presidio, then UNKNOWN
        entity_type = (
            ai_ent.get("entity_type")
            or (best_match.get("entity_type") if best_match else None)
            or "UNKNOWN"
        )
        ...
        results.append(
            {
                "entity": (
                    str(best_match["entity"])
                    if best_match
                    else str(ai_ent.get("entity"))
                ),
                "entity_type": str(entity_type),
                "start": (
                    int(best_match["start"]) if best_match else int(ai_ent.get("start"))
                ),
                "end": int(best_match["end"]) if best_match else int(ai_ent.get("end")),
                "doc_id": str(doc_id) if doc_id is not None else None,
                "presidio_score": (
                    float(presidio_score) if presidio_score is not None else None
                ),
    ...
    for i, pres_ent in enumerate(presidio_entities):
        if i not in used_presidio and str(pres_ent.get("doc_id")) == str(doc_id):
            ...
            entity_type = pres_ent.get("entity_type") or "UNKNOWN"
            results.append(
                {
                    ...
                    "confidence": (
                        "low" if score is not None and score < 0.7 else "medium"
                    ),
                }
            )
...
def align_entities_udf(fuzzy_threshold: int = DEFAULT_FUZZY_MATCH_THRESHOLD):
    ...
    def _align_udf(
        ai_col: pd.Series, presidio_col: pd.Series, doc_id_col: pd.Series
    ) -> pd.Series:
        """Align entities for each row in the batch."""
        return pd.Series(
            [
                align_entities_row(
                    ai, presidio, doc_id, fuzzy_threshold=fuzzy_threshold
                )
