...
def calculate_match_score(
    ...
    """
    # Exact match
    if (entity1.entity == entity2.entity and 
        entity1.start == entity2.start and 
...
class MultiSourceAligner:
    ...
    def align(
        ...
        primary_source = max(
            ...
            key=lambda s: len(normalized_entities[s])
        )
        ...
        for primary_idx, primary_entity in enumerate(normalized_entities[primary_source]):
            ...
            for other_source in ["presidio", "gliner", "ai"]:
                ...
                candidates = [
                    entity for idx, entity in enumerate(normalized_entities[other_source])
            ...
            overall_match_type = (
                MatchType.EXACT if all(mt == MatchType.EXACT for mt in match_types)
                else match_types[0] if len(match_types) == 1
                else MatchType.OVERLAP_FUZZY
            )
