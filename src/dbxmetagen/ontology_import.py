...
def owl_to_bundle_yaml(
    ...
    Requires rdflib. Install with: pip install rdflib
    ...
    Returns:
        The bundle dict (same structure as general.yaml)
    """
    try:
        import rdflib
        ...
        raise ImportError(
            "rdflib is required for ontology import. Install with: pip install rdflib"
        ) from exc
    ...
    # Extract classes as entity definitions
    entities = {}
    for cls in g.subjects(RDF.type, OWL.Class):
        ...
        if parents:
            entities[name]["parent"] = parents[0]
    ...
    # Extract object properties as edges + entity properties
    edge_catalog = {}
    for prop in g.subjects(RDF.type, OWL.ObjectProperty):
        ...
        edge_catalog[prop_name] = {
            "symmetric": False,
            "category": "business",
        }
        if domains:
            edge_catalog[prop_name]["domain"] = domains[0] if len(domains) == 1 else domains
        if ranges:
            edge_catalog[prop_name]["range"] = ranges[0] if len(ranges) == 1 else ranges
        if inverse_props:
            edge_catalog[prop_name]["inverse"] = inverse_props[0]
    ...
    bundle = {
        "metadata": {
            ...
            "version": "3.0",
            "industry": "imported",
            "description": f"Imported from {Path(owl_path).name}",
            "standards_alignment": "OWL",
        },
        "ontology": {
            "version": "3.0",
            "name": f"{bundle_name} Ontology",
            "description": f"Auto-imported from {Path(owl_path).name}",
    ...
    if output_path:
        ...
        logger.info(
            "Wrote bundle YAML to %s (%d entities, %d edges)",
            output_path,
            len(entities),
            len(edge_catalog),
        )
