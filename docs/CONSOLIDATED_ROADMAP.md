...
| ID | Item | Status | Priority | Effort | Source |
|----|------|--------|----------|--------|--------|
| PQ-1 | Data-driven FK candidate generator (value-overlap/containment, name-independent; tiered from CACHED profiling; SR_DATA_OVERLAP lowest trust; federation = LIMIT+collect local compute) | DONE (recall verified live; one precision item open — see PQ-1a) | P1 | L | UAT two-ontology + fk_hard |
| PQ-1a | **OPEN** — 1:1 table-mirror false positive: `dim_customer.customer_name -> dim_customer_staging.customer_name` (a dimension + its exact staging copy) is predicted `is_fk=true` (conf 0.879). Enters via the EMBEDDING-similarity generator (equal names + identical values); joins perfectly (join_rate=1, ri=1) so `never_joins` doesn't catch it. Added a generator-agnostic scoring-time mirror veto (both card ratios >= `fk_mirror_uniqueness_threshold` 0.95 on a low-trust source -> is_fk=false + confidence collapse) but it did NOT fire live despite verified-correct preconditions (both tables profiled, node-id/stat-key formats match, card ratios ~1.0). A debug build (commit e663576) logging the veto's column-presence guard is deployed + a re-run is in flight — read the `predict_foreign_keys` driver log for `"Mirror veto: cols_present="` to see whether `source_rank`/`_card_ratio_a/b` are actually present at `write_predictions`. | OPEN | P1 | S | UAT fk_hard |
