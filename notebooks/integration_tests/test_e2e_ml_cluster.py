...
# MAGIC %md
# MAGIC # E2E Integration Test: ML Cluster (DBR 15.4)
# MAGIC
# MAGIC Tests dbxmetagen end-to-end on ML runtime across all modes.
...
print_test_header("E2E ML Cluster Test")
...
try:
    ...
    test_table = test_utils.create_test_table("test_e2e_ml_cluster", with_comment=False)
    ...
    config_comment = MetadataConfig(
        ...
        control_table="metadata_control_e2e_ml_{0}",
    ...
    control_table_name = (
        f"metadata_control_e2e_ml_{sanitize_user_identifier(current_user)}"
    )
    ...
    test_passed = True
    print_test_result("E2E ML Cluster", True)

except TestFailure as e:
    error_message = str(e)
    print(error_message)
    print_test_result("E2E ML Cluster", False, error_message)
    ...
    try:
        control_table = f"{test_catalog}.{test_schema}.metadata_control_e2e_ml_{sanitize_user_identifier(current_user)}"
